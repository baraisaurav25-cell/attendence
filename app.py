...
def page_attendance():
    ...
    if df.empty:
        st.info("এই section-এ কোনো student নেই। Reports > Section Management থেকে যোগ করুন।")
        return
    ...
    with st.form("att_form"):
        ...
        for _, row in view_df.iterrows():
            ...
            # Current value for this date_key
            cur_val = int(df.loc[df["ID"] == sid, date_key].values[0]) if date_key in df.columns else 0
            default = cur_val == 1
            ...
            with col2:
                attendance_map[sid] = st.checkbox("✔", value=default, key=f"cb_{sid}_{date_key}",
