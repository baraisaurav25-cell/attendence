...
# ─────────────────────────────────────────────
#  USER / AUTH HELPERS
#  ✅ FIX 1: _hash() defined BEFORE DEFAULT_ADMIN
# ─────────────────────────────────────────────
def _hash(pwd: str) -> str:
    return hashlib.sha256(pwd.encode()).hexdigest()

# ✅ FIX 1 (continued): DEFAULT_ADMIN now defined after _hash()
...
def sidebar():
    with st.sidebar:
        ...
        if st.button("🚪 Logout"):
            # ✅ FIX 5: Correct types for each key on logout
...
def page_reports():
    ...
    # ✅ FIX 2: .applymap() → .map() for Pandas 2.x compatibility
    ...
    if st.session_state.role == "admin" and st.checkbox("সব Section-এর combined report"):
        ...
        if all_dfs:
            ...
            # ✅ FIX 3: engine="openpyxl" যোগ করা হয়েছে combined download-এ
            buf2 = BytesIO()
            with pd.ExcelWriter(buf2, engine="openpyxl") as writer:
                combined.to_excel(writer, sheet_name="All Sections", index=False)
