...
def create_section():
    ...
    sec_name = st.text_input("Section Name", key="new_section_name")
    if st.button("Create Section", key="create_section_btn") and sec_name.strip():
...
# =========================
# STUDENT MANAGEMENT (FIXED: UNIQUE KEYS PER SECTION)
# =========================
def student_module(section: str):
    ...
    with st.expander("Add New Student"):
        ...
        if mode == "Manual":
            sid = st.text_input("Student ID", key=f"sid_{section}")
            name = st.text_input("Student Name", key=f"name_{section}")
            if st.button("Add Student", key=f"add_{section}"):
                if sid and name:
                    if sid not in df["ID"].astype(str).values:
                        ...
                        st.error("ID already exists in this section")
                else:
                    st.error("Both ID and Name are required")
        else:
            bulk_text = st.text_area("Enter ID,Name per line (e.g., 101,John Doe)", key=f"bulk_{section}")
            if st.button("Add Bulk", key=f"bulk_btn_{section}"):
                new_rows = []
                duplicates = []
                ...
                if new_rows:
                    ...
                    st.error("No valid rows found or all IDs already exist")
...
def report_page(section: str):
    ...
    if df.empty:
        ...
        if st.button("← Back to Dashboard", key=f"back_report_{section}"):
            st.session_state.page = "dashboard"
            st.rerun()
        return
    ...
    if st.button("← Back to Dashboard", key=f"back2_report_{section}"):
        st.session_state.page = "dashboard"
        st.rerun()
